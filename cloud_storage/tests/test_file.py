...
def save_file_locally_if_no_cloud_storage(file):
    ...
    if not getattr(frappe.conf, "cloud_storage_settings", None) or (
        ...
        and frappe.conf.cloud_storage_settings.get("use_local", False)
    ):
...
@mock_s3
def test_file_versioning_with_content_change(example_csv_file, tmp_path):
    frappe.set_user("Administrator")
    file1 = create_upload_file(example_csv_file, file_name="sample.csv")
    assert frappe.db.exists("File", file1.name)

    modified_csv = tmp_path / "sample.csv"
    with open(example_csv_file) as src, open(modified_csv, "w") as dst:
        lines = src.readlines()
